...
The performance limiter is off by default to reduce dropped notes. When enabled from `SF2 Settings`, voice caps default to `maxVoices: 512` and `maxVoicesPerChannel: 256`; MIDI scheduling still uses `maxMessagesPerTick: 4096`. `playMidiFile()` also accepts `lookaheadMs`, `schedulerIntervalMs`, and `maxEventsPerTick` overrides for tuning. Output gain defaults to `0.3` with a Web Audio limiter; output gain, the performance limiter, and voice limits can be changed from `SF2 Settings`.
